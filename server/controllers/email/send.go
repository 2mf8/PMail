...
var LoginStatus = map[string]*models.User{}
...
func CodeSend(ctx *context.Context, w http.ResponseWriter, req *http.Request) {
	...
	var reqCodeData codeSend
	err = json.Unmarshal(reqBytes, &reqCodeData)
	if err != nil {
		log.WithContext(ctx).Errorf("%+v", err)
		response.NewErrorResponse(response.ParamsError, "params error", err.Error()).FPrint(w)
		return
	...
	if reqCodeData.Token != "" {
		log.WithContext(ctx).Errorf("token error")
		response.NewErrorResponse(response.ParamsError, "params error", "token error").FPrint(w)
		return
	}
	...
	reqData := sendRequest{
		...
		To: []user{
			{
				Email: fmt.Sprintf("%s@qq.com", reqCodeData.SetUin),
				Name:  reqCodeData.SetUin,
			},
		},
		Subject: "2mf8.cn-验证码",
